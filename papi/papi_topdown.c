...
//	}
	...
	total_dispatch_slots=8*total_1;
	frontend_bound=(double)total_3/(double)total_dispatch_slots;
...
//	event_name2=strdup("OPS_SOURCE_DISPATCHED_FROM_DECODER:DECODER:OPCACHE:k=1:u=1");
	event_name2=strdup("perf_raw::r4307aa:u=1:k=1");
...
/******************************************************/
/******************************************************/
/**************** Level2 Backend Bound ****************/
/******************************************************/
/******************************************************/

static int zen5_level2_backend_bound(void) {
	...
	event_name1=strdup("CYCLES_NOT_IN_HALT:u=1:k=1");
	event_name2=strdup("DISPATCH_STALLS_1:BE_STALLS:u=1:k=1");
		/* Fixme: missing completion filter */
//	event_name3=strdup("CYCLES_NO_RETIRE:NOT_COMPLETE_LOAD_AND_ALU:u=1:k=1");
	event_name3=strdup("perf_raw::r43a2d6:u=1:k=1");
	event_name4=strdup("CYCLES_NO_RETIRE:NOT_COMPLETE_LOAD_AND_ALU:u=1:k=1");

	printf("Level 2 Backend Bound\n");
...
//	}
	...
	long long total_dispatch_slots;
	double backend_bound;
	double backend_bound_memory;
	double backend_bound_cpu;

	total_dispatch_slots=8*total_1;
	backend_bound=(total_2)/(double)total_dispatch_slots;
	backend_bound_memory=backend_bound*((double)total_3/total_4);
	backend_bound_cpu=backend_bound*(1.0-((double)total_3/total_4));
...
//		printf("\traw: %lld %lld %lld %lld\n",total_1,total_2,total_3,total_4);
		printf("\tTotal Dispatch Slots = %lld\n",total_dispatch_slots);
		printf("\tBackend Bound = %.2lf%%\n",backend_bound*100.0);
		printf("\tBackend Bound Memory = %.2lf%%\n",backend_bound_memory*100.0);
		printf("\tBackend Bound CPU = %.2lf%%\n",backend_bound_cpu*100.0);
	}
...
static int zen5_level2_retiring(void) {
	...
	event_name1=strdup("CYCLES_NOT_IN_HALT:u=1:k=1");
	event_name2=strdup("RETIRED_OPS:k=1:u=1");
	event_name3=strdup("RETIRED_UCODE_OPS:k=1:u=1");

	printf("Level 2 Retiring\n");
...
//	}
	...
	long long total_dispatch_slots;
	double retiring;
	double retiring_fastpath;
	double retiring_microcode;

	total_dispatch_slots=8*total_1;
	retiring=(total_2)/(double)total_dispatch_slots;
	retiring_fastpath=retiring*(total_2-total_3)/(double)(total_2);
	retiring_microcode=retiring*(total_3)/(double)(total_2);

	if (!quiet) {
		printf("\tRetiring = %.2lf%%\n",retiring*100.0);
		printf("\tRetiring Fastpath = %.2lf%%\n",retiring_fastpath*100.0);
		printf("\tRetiring Microcode = %.2lf%%\n",retiring_microcode*100.0);
	}
...
int main(int argc, char **argv) {
	...
	zen5_level2_backend_bound();
	zen5_level2_retiring();
