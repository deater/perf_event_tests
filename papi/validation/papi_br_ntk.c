...
int main(int argc, char **argv) {
   ...
   char test_string[]="Testing PAPI_BR_NTK predefined event...";

   quiet=test_quiet();

   retval = PAPI_library_init(PAPI_VER_CURRENT);
   if (retval != PAPI_VER_CURRENT) {
      if (!quiet) printf("Erro: PAPI_library_init\n");
      test_fail(test_string);
   }

   retval = PAPI_query_event(PAPI_BR_NTK);
   if (retval != PAPI_OK) {
      if (!quiet) printf("PAPI_BR_NTK not supported\n");
      test_skip(test_string);
   }

   if (!quiet) {
      printf("\n");
      printf("Testing a loop with %lld not-taken branches (%d times):\n",
          expected,num_runs);
   }
