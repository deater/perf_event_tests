/* This file attempts to test the retired conditional branches */
/* performance counter on various architectures, as            */
/* implemented by the PAPI_BR_CN counter.                      */
...
int main(int argc, char **argv) {
  ...
  int retval,quiet;
   ...
   retval = PAPI_query_event(PAPI_BR_CN);
   if (retval != PAPI_OK) {
      if (!quiet) printf("ERROR: PAPI_BR_CN not supported %d\n", retval);
   ...
   if ((error > 1.0) || (error<-1.0)) {
      ...
      if (!quiet) printf("ERROR: Instruction count off by more than 1%%\n");
      test_fail(test_string);
   }
   if (!quiet) printf("\n");
