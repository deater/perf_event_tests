/* This code runs some sanity checks on the PAPI_L2_TCR */
/*   (L2 Total Cache Reads) count.                      */
...
int main(int argc, char **argv) {
   ...
   char test_string[]="Testing PAPI_L2_TCR predefined event...";
   ...
   retval = PAPI_query_event(PAPI_L2_TCR);
   if (retval != PAPI_OK) {
      if (!quiet) printf("PAPI_L2_TCR not available");
   ...
   if (array==NULL) {
     if (!quiet) printf("Can't allocate memory\n");      
      test_fail(test_string);
   }
