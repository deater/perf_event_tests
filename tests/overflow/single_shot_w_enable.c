/* single_shot_w_enable.c  */
/* by Vince Weaver   vincent.weaver _at_ maine.edu */

/* Test different signal sources from enable vs refresh */
...
int main(int argc, char** argv) {
   ...
   struct sigaction sa;
   void *our_mmap;
   char test_string[]="Testing enable vs refresh signal types...";
   
   quiet=test_quiet();

   if (!quiet) printf("This tests the signal type with enable vs refresh.\n");
   ...
   if (count.in!=1) {
      if (!quiet) printf("Expected 1 POLL_IN interrupt.\n");
      test_fail(test_string);
   }
