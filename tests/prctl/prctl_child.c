...
/*  particularly, if local prctl() starts/stops events in attacher */
...
//	kill( getpid(), SIGSTOP );
	...
	if (!quiet) {
		printf("CHILD: waiting to start events\n");
	}
	...
	if (!quiet) {
		printf("CHILD: events_started\n");
	}

	if (!quiet) {
		printf("+ Starting counting with prctl in child\n");
	}
	...
	if (!quiet) {
		printf("+ Stopping counting with prctl in child\n");
	}
	...
	if (!quiet) {
		printf("\tCHILD Read (should not be 0): %lld\n",insn_data[0]);
	}
