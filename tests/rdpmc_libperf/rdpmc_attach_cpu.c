...
int main(int argc, char **argv) {
	...
	int cpu,idx,err;
	int i,result,num_cpus=1,current_cpu;
	double error;
	...
	idx=0;
	perf_evlist__for_each_evsel(evlist, evsel) {
		err = perf_evsel__read(evsel, 0, 0, &counts[idx]);
		if (err) {
			if (!quiet) {
	...
	if (!quiet) {
		printf("\tEvent %d -- Raw count: %lu"
			"enabled: %lu running: %lu\n",
	...
	perf_evlist__for_each_evsel(evlist, evsel) {
		perf_cpu_map__for_each_cpu(cpu, idx, cpus) {
			if (!quiet) {
				if (counts[idx].val == 0) {
					fprintf(stderr, "\tCPU %d -- rdpmc support not available.\n", cpu);
				}
				else {
